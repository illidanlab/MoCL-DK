...
class DrugData(InMemoryDataset):
    def __init__(self, root, cl,
        ...
        self.rdkit_mol_objs = rdkit_mol_objs
        self.labels = labels
        self.training = training

        super(DrugData, self).__init__(root, transform, pre_transform, pre_filter)
        self.transform, self.pre_transform, self.pre_filter = transform, pre_transform, pre_filter

        if not empty:
            self.data, self.slices = torch.load('./tmp/' + cl + '/processed_data_' + str(training) + '.pt')
    ...
    def process(self):
        ...
        smiles_list, rdkit_mol_objs, labels = self.smiles_list, self.rdkit_mol_objs, self.labels
        ...
        torch.save((data, slices), './tmp/processed_data_' + str(self.training) + '.pt')
